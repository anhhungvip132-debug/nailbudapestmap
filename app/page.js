...
const PromoSlider = dynamic(
  () => import("../components/ui/PromoSlider"),
  {
    ssr: false,
    loading: () => (
      <p className="text-center py-6">Đang tải khuyến mãi…</p>
    ),
  }
);

const FeaturedAds = dynamic(
  () => import("../components/ui/FeaturedAds"),
  {
    ssr: false,
    loading: () => (
...
const ChatWidget = dynamic(
  () => import("../components/ui/ChatWidget.jsx"),
  { ssr: false }
);

const Map = dynamic(() => import("../components/ui/Map.jsx"), {
  ssr: false,
});

const NearestSalons = dynamic(
  () => import("../components/ui/NearestSalons.jsx"),
  { ssr: false }
);
...
function BlogSection() {
  return (
    <section
      id="blog"
      className="max-w-7xl mx-auto mt-20 px-4 mb-20"
    >
      ...
      <div className="flex flex-col md:flex-row items-center gap-8 mb-12 bg-white p-6 rounded-2xl shadow">
        ...
        <div className="md:w-1/2">
          <h3 className="font-bold text-2xl mb-3">
            Nail Trends in Budapest
          </h3>
          <p className="text-gray-600">
            Các xu hướng nail hot nhất Budapest.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row items-center gap-8 bg-white p-6 rounded-2xl shadow">
        <div className="md:w-1/2 order-2 md:order-1">
          ...
          <p className="text-gray-600">
            Những mẫu nail tuyệt đẹp dành cho các mùa lễ hội &amp; sự
            kiện.
          </p>
        </div>
...
export default function HomePage() {
  ...
  // filter từ SearchBar (để dùng sau này nếu muốn)
  ...
  // handler nhận filter từ SearchBar.jsx
  const handleFilterChange = (filter) => {
    // Bạn có thể log để kiểm tra:
    console.log("Filter từ SearchBar:", filter);
    setSearchFilters(filter);
    // TODO: nếu muốn lọc danh sách thật, có thể gọi API /api/salons ở đây
  };
  ...
  return (
    <div>
      ...
      {/* ⭐ THANH TÌM KIẾM – ĐÃ TRUYỀN onFilter ĐÚNG KIỂU */}
...
/* ====== FEATURED ADS LIST (URL UNSPLASH ĐÃ SỬA) ====== */
const featuredList = [
  {
    id: 1,
